...
#         await state.update_data(data_recipes = recipes)
#         if len(recipes) <= 0:
...
#         async def query_show_list(query: types.CallbackQuery, callback_data: dict,state_cat: FSMContext):
#             history_recipe = bd.bot_get_history(msg_for_search.from_user.id)[-1]
#             last_recipes = await state.get_data()
#             reply_fmt = get_reply_fmt(last_recipes['data_recipes'], callback_data['start_indx'])
#             await query.message.edit_text(reply_fmt['msg'], reply_markup=reply_fmt['markup'])
#             await state_cat.finish()
#
#         @dp.callback_query_handler(recipe_cb.filter(action='view'),state=SearchByCategories.waiting_for_user_view)
#         async def query_view(query: types.CallbackQuery, callback_data: dict,state_cat: FSMContext):
...
#         async def query_change_fav(query: types.CallbackQuery, callback_data: dict,state_cat: FSMContext):
#             recipe_id = callback_data['recipe_id']
...
#
#
# @dp.message_handler(Text(equals='Хочу найти рецепты с ингредиентами...'), state="*")
...
#             await query.message.edit_text(text, reply_markup=markup)
#
#         @dp.callback_query_handler(recipe_cb.filter(action=['unfavourite', 'favourite']),state=SearchByIngredients.waiting_for_user_view)
