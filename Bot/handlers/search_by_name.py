...
@dp.message_handler(Text(equals='Хочу найти рецепты с названием...'),state="*")
async def search_names(msg_search_type: types.Message):
    await SearchByName.waiting_for_user_answer.set()
    ...
    async def user_answer_handler(msg_for_search: types.Message, ):
        ...
        @dp.callback_query_handler(recipe_cb.filter(action=['unfavourite', 'favourite']),state=SearchByName.waiting_for_user_view)
